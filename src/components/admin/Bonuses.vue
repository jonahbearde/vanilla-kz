<template>
	<div class="flex flex-1 flex-col">
		...
		<div class="flex w-full items-center justify-center text-xl border-b border-black py-2">
			<input @input="getSearchResults" v-model="bonusSearch" type="text" placeholder="search bonuses"
		...
		<div class="flex w-full justify-center max-h-[600px] overflow-auto py-4 border-b border-black">
			...
			<table class="w-5/6 text-center border-collapse">
				...
				<tbody>
					<tr v-for="bonus in bonuses" :key="bonus.id" @click="edit(bonus)"
		...
		<div v-if="showEditor" class="px-8 py-4">
			<div class="flex items-center gap-16">
				...
				<div class="flex items-baseline gap-2 text-xl">
					...
					<input v-model="bonusNumberToSubmit" type="text"
			...
			<div class="py-4">
				...
				<div class="flex items-center gap-2 text-xl">
					...
					<button v-if="editing" @click="deletebonus"
...
const getPossibleBonuses = async () => {
	try {
		const bonusesData = await axios.get(`https://vnl-stats-backend.onrender.com/api/v1/bonus`);
...
const getSearchResults = () => {
	...
	if (bonusSearch.value !== '') {
		...
		bonuses.value = allBonuses.filter(bonus => {
...
const submitChange = async () => {
	try {
		addError.value = '';
		if (!mapToSubmit.value || !bonusNumberToSubmit.value || !tpTierToSubmit.value || !proTierToSubmit.value || !notesToSubmit.value) {
		...
		const upsertResult = await axios.put(`https://vnl-stats-backend.onrender.com/api/v1/bonus/${bonusId}`, bonusObj);
...
const deletebonus = async () => {
	...
	if (confirm('do you want to delete this bonus?')) {
		try {
			await axios.delete(`https://vnl-stats-backend.onrender.com/api/v1/bonus/${bonus.id}`);
			await getPossibleBonuses();
